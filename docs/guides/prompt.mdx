...
**Learn the ins and outs of writing great prompts to get the most out of Paradigm**.

:::caution
The use of ambiguous, disrespectful, racist, or otherwise improper vocabulary can lead to improper generations. Please use common sense when generating text. LightOn is not responsible for improper use of Paradigm.
:::
...
In this guide, we review different types of prompts which can be used for Paradigm.

## 📢 Give instructions

Write a description, in natural language, of the task you want Paradigm to perform.

For example, we can ask Paradigm to generate an Instagram ad for a resort.

:::tip

Write a task description as detailed as possible. Paradigm will utilize the details to perform the task at its best.
...
In the first prompt, we offer little detail on the resort that we are trying to advertise. As a result, the advertisement is generic, Paradigm talks about the Mediterranean sea, that might not be the location of the resort, 
and there are some repetitions of the word *atmosphere* as that is the only item Paradigm could pick up from the prompt. Compare this with the result from the second prompt, where we provided details on the location
and the main selling points of the resort: the advertisement generated talks about the Philippines, scuba diving and luxurious villas. The overall quality of the generation is improved.

## 🪞 Add examples

**Adding examples to the prompt**, especially if the task is more complex, **improves the generation**. Paradigm will understand and mirror the behavior. For example, we can use existing answer to reviews to improve our automated customer service.
However, when adding examples,

:::tip

Provide examples in your prompt on top of the task description. Adding one or more examples of how you would like the task to be carried out will help Paradigm perform better. You can separate examples with 
sequences like `***`, `---` or `###`.
...
The prompt on the left offers no example of how to reply, therefore Paradigm is confused and generates a similar review instead of a reply. In the prompt on the right, we offer one example of answering a review. 
Paradigm picks up on the key elements: thank the client for the review, and ask them to come back soon. 
...
What happens if we provide a negative review? Paradigm might default to the generic reply for positive reviews, and clearly that is something we want to avoid. 
...
In the prompt on the left, Paradigm defaults to a positive reply even if the review is negative. In the prompt on the right we now offer an additional example of reply to a negative review, and Paradigm is able to reply
picking up the item that was not liked by the customer.

## ⚖️ Calibrate and order the examples

Imagine that we now want to utilize Paradigm to classify review in positive and negative. What happens when we add six examples with five positive classification and only one negative? 
Or when the split is 50/50, but we put first the three negative ones and then the three positive ones? In both cases Paradigm will be biased to classify into the *positive* category. Paradigm picks up on
...
In the left prompt we have five examples out of six that are positive reviews, and the only negative example is at the beginning of the prompt. As a result, Paradigm misclassified the last review as being positive.
In the box on the right, we improve the prompt by using three negative and three positive examples, and alternating them regularly. Paradigm now classifies correctly our review!

## 📚  Spelling, grammar, formatting

Paradigm works by completing an input text as if it was a document to continue, and documents rarely change writing style abruptly. If your prompt contains spelling, grammar errors or inconsistent formatting, completions will also 
...
Avoid spelling, grammar errors or inconsistent formatting if you do not want these issues to appear in the Paradigm completions. Poorly written documents are rarely well-reasoned.
...
Instead of simply providing examples of inputs and outputs of a task, you also provide the reasoning behind the output in each example. Paradigm will utilize this information to provide more accurate completions.
